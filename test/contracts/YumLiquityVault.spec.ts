...
import { YumLusdVault } from "../../types/YumLusdVault";
...
let YumLUSDVaultFactory: ContractFactory;
...
describe("YumLUSDVault", () => {
  let signers: Signer[];

  before(async () => {
    YumLUSDVaultFactory = await ethers.getContractFactory("YumLUSDVault");
  ...
  describe("constructor", async () => {
    ...
    context("when governance is the zero address", () => {
      it("reverts", async () => {
        expect(
          YumLUSDVaultFactory.connect(deployer).deploy(
            token.address,
            alUsd.address,
            ZERO_ADDRESS,
            await sentinel.getAddress()
          )
        ).revertedWith("YumLUSDVault: governance address cannot be 0x0.");
      });
    });
  });
  ...
  describe("update Alchemist addys and variables", () => {
    ...
    beforeEach(async () => {
      ...
      alchemist = (await YumLUSDVaultFactory.connect(deployer).deploy(
        token.address,
        alUsd.address,
    ...
    describe("set governance", () => {
      context("when caller is not current governance", () => {
        ...
        it("reverts", async () => {
          ...
          ).revertedWith("YumLUSDVault: only governance");
        });
      });
      ...
      context("when caller is current governance", () => {
        ...
        it("reverts when setting governance to zero address", async () => {
          expect(alchemist.setPendingGovernance(ZERO_ADDRESS)).revertedWith(
            "YumLUSDVault: governance address cannot be 0x0."
          );
        });
      ...
      context("when caller is current governance", () => {
        ...
        it("reverts when setting transmuter to zero address", async () => {
          expect(alchemist.setTransmuter(ZERO_ADDRESS)).revertedWith(
            "YumLUSDVault: transmuter address cannot be 0x0."
          );
        });
      ...
      context("when caller is current governance", () => {
        ...
        it("reverts when setting rewards to zero address", async () => {
          expect(alchemist.setRewards(ZERO_ADDRESS)).revertedWith(
            "YumLUSDVault: rewards address cannot be 0x0."
          );
        });
      ...
      context("when caller is current governance", () => {
        ...
        it("reverts when performance fee greater than maximum", async () => {
          ...
          expect(alchemist.setHarvestFee(MAXIMUM_VALUE.add(1))).revertedWith(
            "YumLUSDVault: harvest fee above maximum"
          );
        });
    ...
    describe("set collateralization limit", () => {
      ...
      context("when caller is current governance", () => {
        ...
        it("reverts when performance fee less than minimum", async () => {
          ...
          ).revertedWith("YumLUSDVault: collateralization limit below minimum.");
        });
        ...
        it("reverts when performance fee greater than maximum", async () => {
          ...
          ).revertedWith("YumLUSDVault: collateralization limit above maximum");
        });
  ...
  describe("vault actions", () => {
        ...
        context("when adapter is zero address", async () => {
          it("reverts", async () => {
            expect(alchemist.migrate(ZERO_ADDRESS)).revertedWith(
              "YumLUSDVault: active vault address cannot be 0x0."
            );
          });
        });
    ...
    describe("flush funds", () => {
      ...
      context("when the Alchemist is not initialized", () => {
        it("reverts", async () => {
          expect(alchemist.flush()).revertedWith("YumLUSDVault: not initialized.");
        });
      });
